...
export default function CommandPalette({ isOpen, onClose }: CommandPaletteProps) {
  ...
  const staticCommands = useMemo<Command[]>(() => {
    ...
    cmds.push({
      ...
      subtitle: `Currently in ${theme} mode (Shortcut: T)`,
      icon: theme === "dark" ? <Sun className="h-4 w-4 text-amber-400" /> : <Moon className="h-4 w-4 text-sky-400" />,
      category: "actions",
      action: () => {
        toggleTheme();
        onClose();
                          ...
                          >
                            <span
                              className={cn(
                                ...
                                isSelected
                                  ? "bg-violet-500 text-white light:text-slate-950"
                            ...
                            {cmd.category === "social" ? (
                              ...
                              <ArrowRight
                                className={cn(
                                  ...
                                  isSelected
                                    ? "translate-x-1 text-violet-400 light:text-violet-600"
                                    : "text-slate-500"
                                )}
                              />
                            )}

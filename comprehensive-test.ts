...
const BASE_URL = "https://jeffreyemanuel.com";
...
async function testPage(
  ...
  try {
    ...
    // Check for broken layouts - content should be visible
    const bodyWidth = await page.evaluate(() => document.body.scrollWidth);
    const viewportWidth = viewport.width;

    if (bodyWidth > viewportWidth + 20) {
      warnings.push(`Horizontal overflow detected: body=${bodyWidth}px, viewport=${viewportWidth}px`);
    }
    ...
    if (pageName === "projects" && viewportName === "mobile") {
      const flywheelVisible = await page.evaluate(() => {
        const flywheel = document.querySelector('[aria-label="Interactive flywheel showing tool connections"]');
...
async function runTests() {
  ...
  try {
    for (const [viewportName, viewport] of Object.entries(VIEWPORTS)) {
      ...
      for (const pageInfo of PAGES) {
        const result = await testPage(context, pageInfo.path, pageInfo.name, viewportName, viewport);
        results.push(result);
      }
  ...
  try {
    ...
    const filterButtons = interactivePage.locator('nav[aria-label="Filter projects by category"] button');
    ...
    // Test scroll to top button (if visible)

...
async function supportsFinePointer(page: Page) {
...
async function isToolCardInViewport(page: Page, toolId: string) {
...
async function waitForToolCardNavigation(page: Page, toolId: string, initialScroll: number) {
  return expect
    .poll(
      async () => {
        const visible = await isToolCardInViewport(page, toolId);
        if (visible) {
          return true;
        }

        const currentScroll = await page.evaluate(() => window.scrollY);
        return Math.abs(currentScroll - initialScroll) > 20;
      },
      { timeout: 6000, intervals: [50, 100, 150, 250, 500] }
    )
    .toBe(true);
}
...
async function isToolCardHighlighted(page: Page, toolId: string) {
  const selector = `#tool-card-${toolId} [data-testid="tool-card"], #tool-card-${toolId}`;
...
test.describe("TLDR Page - Interactive Synergy Diagram", () => {
  ...
  test.describe("Basic Rendering", () => {
    ...
    test("should have aria-labels on tool nodes", async ({ page }) => {
      ...
      const ntmNode = diagram.locator('[role="button"][aria-label*="NTM"]');
    ...
    test("should display connection lines between tools", async ({ page }) => {
      const diagram = page.locator('svg[aria-label*="synergy diagram"]');
  ...
  test.describe("Hover Interactions", () => {
    test("should dim non-connected nodes when one is hovered", async ({ page }) => {
      console.log("[E2E:Diagram] Testing hover dimming");
      const diagram = page.locator('svg[aria-label*="synergy diagram"]');

      // Find a node to interact with
      const ntmNode = diagram.locator('[role="button"][aria-label*="NTM"]');
      const hasFinePointerMatch = await supportsFinePointer(page);

      const measureDimmedNodes = async () => {
          ...
          async () => {
            ...
            const opacities = await Promise.all(
              Array.from({ length: nodeCount }, (_, index) =>
                allNodes.nth(index).evaluate((el) =>
                  parseFloat(getComputedStyle(el).opacity)
                )
              )
            );

            return opacities.every((value) => value >= 0.9);
          },
          { timeout: 2000, intervals: [50, 100, 200, 300] }
        )
        .toBe(true);
  ...
  test.describe("Click Navigation", () => {
    test("should scroll to tool card when node is clicked", async ({ page }) => {
      ...
      await waitForToolCardNavigation(page, "ntm", initialScroll);
    ...
    test("should briefly highlight target card after navigation", async ({
      ...
      console.log("[E2E:Diagram] Cass card highlight confirmed");
      ...
      await expect
        .poll(
          ...
          { timeout: 2500, intervals: [150, 250, 400, 500] }
        )
        .toBe(false);

      console.log("[E2E:Diagram] Highlight removed after timeout");
    });
  });
  ...
  test.describe("Reduced Motion", () => {
    test("should still function with prefers-reduced-motion", async ({
      ...
      const ntmNode = diagram.locator('[role="button"][aria-label*="NTM"]');
      const reducedMotionInitialScroll = await page.evaluate(() => window.scrollY);
      await ntmNode.click();
      const reducedMotionScroll = await page.evaluate(() => window.scrollY);
      expect(reducedMotionScroll).not.toBe(reducedMotionInitialScroll);

      // Should still navigate to card
      const ntmCard = page
        .locator('[data-testid="tool-card"]')
        .filter({ hasText: /Named Tmux Manager/i });
      await expect(ntmCard).toBeVisible();
